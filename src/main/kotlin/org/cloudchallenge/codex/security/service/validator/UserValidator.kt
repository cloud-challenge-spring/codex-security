package org.cloudchallenge.codex.security.service.validator

import org.cloudchallenge.codex.security.model.User
import org.springframework.stereotype.Service
import java.util.regex.Pattern

@Service
class UserValidator {
    fun validate(user: User) {
        user.email = user.email?.toLowerCase()

        if (user.email == null || !checkEmail(user.email!!))
            throw IllegalArgumentException("Email format for: ${user.email} is not acceptable")
        if (user.password != null && user.password!!.length < 8)
            throw IllegalArgumentException("Password must be at least 8 characters long")
    }

    fun checkEmail(email: String): Boolean {
        val regex = "(?:[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*|\"" +
                "(?:[\\x01-\\x08\\x0b\\x0c\\x0e-\\x1f\\x21\\x23-\\x5b\\x5d-\\x7f]|\\\\[\\x01-\\x09\\x0b\\x0c\\x0e-\\x7f])*\")" +
                "@(?:(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?|\\" +
                "[(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?|[a-z0-9-]*" +
                "[a-z0-9]:(?:[\\x01-\\x08\\x0b\\x0c\\x0e-\\x1f\\x21-\\x5a\\x53-\\x7f]|\\\\[\\x01-\\x09\\x0b\\x0c\\x0e-\\x7f])+)\\])"

        val matcher = Pattern.compile(regex).matcher(email)

        return matcher.matches()
    }
}